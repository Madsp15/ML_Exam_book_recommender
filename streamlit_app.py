...
with st.sidebar:
    ...
    llm_provider = st.selectbox(
        "LLM Provider",
        options=["google", "mistral", "cerebras"],
        index=0,
...
if not api_key:
    st.markdown(f'<div class="error-box">⚠️ <strong>Error:</strong> {env_var_name} not found in environment variables. Please add it to your .env file.</div>', unsafe_allow_html=True)
    st.stop()
...
if st.session_state.current_result:
    ...
    if result.get("error"):
        ...
        if result.get("librarian_result"):
            ...
            st.markdown('<div class="info-box">ℹ️ No results found</div>', unsafe_allow_html=True)
